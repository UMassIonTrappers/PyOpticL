...
def _bounding_box(
    ...
    if hasattr(obj, "RelativePlacement"):
        ...
        temp = obj
        while hasattr(temp, "ParentObject") and hasattr(
            temp.ParentObject, "RelativePlacement"
        ):
    ...
    x_min, x_max = (
        bound.XMin - tol * x_tol + min_offset[0],
        bound.XMax + tol * x_tol + max_offset[0],
    )
    y_min, y_max = (
        bound.YMin - tol * y_tol + min_offset[1],
        bound.YMax + tol * y_tol + max_offset[1],
    )
    z_min = (
        min(
            global_bound.ZMin - tol * z_tol + min_offset[2],
            -layout.inch / 2 + plate_off,
        )
        - global_bound.ZMin
        + bound.ZMin
    )
    z_max = (
        max(
            global_bound.ZMax + tol * z_tol + max_offset[2],
            -layout.inch / 2 + plate_off,
        )
        - global_bound.ZMax
        + bound.ZMax
    )
    bound_part = _custom_box(
        dx=x_max - x_min,
        dy=y_max - y_min,
        dz=z_max - z_min,
        x=x_min,
...
def _add_linked_object(
    obj, obj_name, obj_class, pos_offset=(0, 0, 0), rot_offset=(0, 0, 0), **args
):
    new_obj = App.ActiveDocument.addObject(obj_class.type, obj_name)
    new_obj.addProperty(
        "App::PropertyLinkHidden", "Baseplate"
...
def _custom_cylinder(
    dia, dz, x, y, z, head_dia=0, head_dz=0, dir=(0, 0, -1), countersink=False
):
    ...
    if head_dia != 0 and head_dz != 0:
        if countersink:
            ...
            part = part.fuse(
                Part.makeCylinder(
                    head_dia / 2, head_dz, App.Vector(0, 0, 0), App.Vector(*dir)
                )
            )
...
class DiscOptic:
    """
    Defines a disc in space

    Args:
        name (string)
        position (App.Vector): The vector location of the optical center
        normal (App.Vector): The normal vector of the disc
        radius (float): The radius of the disc
    """

    type = "Part::FeaturePython"

    def __init__(self, name, position, normal, radius):
    ...
    def execute(self, obj):
        part = Part.makeCylinder(
            self.obj.Radius,
            self.obj.Thickness,
            [self.obj.Position, self.obj.Vec2.negative()],
        )

        part = _custom_cylinder(
            dia=obj.Diameter.Value,
            dz=obj.Thickness.Value,
            x=0,
            y=0,
            z=0,
            dir=(-1, 0, 0),
...
class example_component:
    ...
    def execute(self, obj):
        part = _custom_box(
            dx=obj.Side_Length.Value,
            dy=obj.Side_Length.Value,
            dz=obj.Side_Length.Value,
            x=0,
            y=0,
            z=self.mount_dz,
        )
        part = part.cut(
            _custom_cylinder(
                dia=self.mount_bolt["clear_dia"],
                dz=obj.Side_Length.Value,
                head_dia=self.mount_bolt["head_dia"],
                head_dz=self.mount_bolt["head_dz"],
                x=0,
                y=0,
                z=obj.Side_Length.Value + self.mount_dz,
            )
        )
        obj.Shape = part
        ...
        part = _custom_cylinder(
            dia=self.mount_bolt["tap_dia"], dz=drill_depth, x=0, y=0, z=self.mount_dz
        )
...
class baseplate_mount:
    ...
    def execute(self, obj):
        ...
        part = _custom_cylinder(
            ...
            dz=bolt_len,
            head_dia=bolt_14_20["head_dia"],
            head_dz=bolt_14_20["head_dz"],
            x=0,
            y=0,
            z=-INCH * 3 / 2 + bolt_len,
        )
        ...
        part = _custom_cylinder(
            dia=bolt_14_20["clear_dia"],
            dz=drill_depth,
            head_dia=bolt_14_20["washer_dia"],
            head_dz=obj.BoreDepth.Value,
            x=0,
            y=0,
            z=-obj.Baseplate.OpticsDz.Value,
        )
        part.Placement = obj.Placement
        obj.DrillPart = part
...
class surface_adapter_lying_down:
    ...
    def __init__(
        self, obj, drill=True, mount_hole_dy=20, adapter_height=8, outer_thickness=2
    ):
        obj.Proxy = self
        ViewProvider(obj.ViewObject)
        ...
        obj.addProperty("App::PropertyBool", "Drill").Drill = drill
        obj.addProperty(
            "App::PropertyLength", "MountHoleDistance"
        ).MountHoleDistance = mount_hole_dy
        obj.addProperty(
            "App::PropertyLength", "AdapterHeight"
        ).AdapterHeight = adapter_height
        obj.addProperty(
            "App::PropertyLength", "OuterThickness"
    ...
    def execute(self, obj):
        ...
        part = part.cut(
            _custom_cylinder(
                dia=bolt_8_32["clear_dia"],
                dz=dz,
                head_dia=bolt_8_32["head_dia"],
                head_dz=bolt_8_32["head_dz"],
                x=0,
                y=0,
                z=-dz,
                dir=(0, 0, 1),
...
class skate_mount_lying_down:
    ...
    def __init__(
        ...
        obj.addProperty("App::PropertyLength", "CubeDepth").CubeDepth = (
            cube_depth + 1e-3
        )
        obj.addProperty(
            "App::PropertyLength", "OuterThickness"
    ...
    def execute(self, obj):
        ...
        dy = dx + obj.MountHoleDistance.Value
        raw_dz = (
            obj.Baseplate.OpticsDz.Value - obj.CubeDz.Value / 2 + obj.CubeDepth.Value
        )
        dz = max(raw_dz, 8)
        ...
        part = _custom_box(
            dx=dx, dy=dy, dz=dz, x=0, y=0, z=-obj.Baseplate.OpticsDz.Value, fillet=5
        )
        part = part.cut(
            _custom_box(
                dx=cut_dx,
                dy=cut_dy,
                dz=obj.CubeDepth.Value + 1e-3,
                x=0,
                y=0,
        ...
        for i in [-1, 1]:
            if obj.Slots:
                part = part.cut(
                    _custom_box(
                        dx=slot + bolt_8_32["head_dia"],
                        dy=bolt_8_32["head_dia"],
                        dz=bolt_8_32["head_dz"],
                        x=0,
                        y=i * obj.MountHoleDistance.Value / 2,
                        z=-obj.Baseplate.OpticsDz.Value + dz,
                        fillet=bolt_8_32["head_dia"] / 2,
                        dir=(0, 0, -1),
                    )
                )
                part = part.cut(
                    _custom_box(
                        dx=slot + bolt_8_32["clear_dia"],
                        dy=bolt_8_32["clear_dia"],
                        dz=bolt_8_32["head_dz"],
                        x=0,
                        y=i * obj.MountHoleDistance.Value / 2,
                        z=-obj.Baseplate.OpticsDz.Value + dz - bolt_8_32["head_dz"],
                        fillet=bolt_8_32["clear_dia"] / 2,
                        dir=(0, 0, -1),
                    )
                )
            else:
        ...
        part = _bounding_box(
            ...
            min_offset=(0, 0, 5),
            max_offset=(0, 0, 5),
        )  # ,min_offset=(-slot, 0, 0), max_offset=(slot, 0, 0))
...
class skate_mount:
    ...
    def execute(self, obj):
        ...
        part = _bounding_box(
            obj, 1, 6, min_offset=(-slot, 0, 0), max_offset=(slot, 0, 0)
        )
        for i in [-1, 1]:
            part = part.fuse(
                _custom_cylinder(
...
class slide_mount:
    ...
    def __init__(
        ...
        obj.addProperty(
            "App::PropertyDistance", "DrillOffset"
        ).DrillOffset = drill_offset
        obj.addProperty(
            "App::PropertyLength", "AdapterHeight"
        ).AdapterHeight = adapter_height
        obj.addProperty(
            "App::PropertyLength", "PostThickness"
        ).PostThickness = post_thickness
        obj.addProperty(
            "App::PropertyLength", "OuterThickness"
        ).OuterThickness = outer_thickness
        obj.addProperty("Part::PropertyPartShape", "DrillPart")
    ...
    def execute(self, obj):
        ...
        part = part.cut(
            _custom_box(
                dx=bolt_8_32["clear_dia"],
                dy=obj.SlotLength.Value + bolt_8_32["clear_dia"],
                dz=dz,
                x=0,
                y=-dy / 2 - obj.PostThickness.Value / 2,
                z=-obj.Baseplate.OpticsDz.Value,
                fillet=bolt_8_32["clear_dia"] / 2,
            )
        )
        part = part.cut(
            _custom_box(
                dx=bolt_8_32["head_dia"],
                dy=obj.SlotLength.Value + bolt_8_32["head_dia"],
                dz=bolt_8_32["head_dz"],
                x=0,
                y=-dy / 2 - obj.PostThickness.Value / 2,
                z=-obj.Baseplate.OpticsDz.Value + bolt_8_32["head_dz"],
                fillet=bolt_8_32["head_dia"] / 2,
            )
        )
        part = part.fuse(
            _custom_box(
                dx=dx,
                dy=obj.PostThickness.Value,
                dz=obj.Baseplate.OpticsDz.Value + bolt_8_32["head_dz"],
                x=0,
                y=-obj.PostThickness.Value / 2,
                z=-obj.Baseplate.OpticsDz.Value,
            )
        )
        part = part.cut(
        ...
        part = _custom_cylinder(
            ...
            dz=drill_depth,
            x=0,
            y=-dy / 2 - obj.PostThickness.Value / 2 + obj.DrillOffset.Value,
            z=-obj.Baseplate.OpticsDz.Value,
        )
        part.Placement = obj.Placement
        obj.DrillPart = part
...
class rotation_stage_rsp05:
    ...
    def __init__(self, obj, invert=False, adapter_args=dict()):
        ...
        _add_linked_object(
            ...
            "Surface Adapter",
            surface_adapter,
            pos_offset=(1.397, 0, -13.97),
            rot_offset=(0, 0, 90 * obj.Invert),
            **adapter_args,
        )
...
class mirror_mount_k05s2:
    ...
    def execute(self, obj):
        mesh = _import_stl(
            "POLARIS-K05S2-Step.stl", (90, -0, -90), (-4.514, 0.254, -0.254)
        )
        mesh.Placement = obj.Mesh.Placement
        obj.Mesh = mesh

        part = _custom_cylinder(
            dia=bolt_8_32["tap_dia"], dz=drill_depth, x=-8.017, y=0, z=-layout.inch / 2
        )
        for i in [-1, 1]:
            part = part.fuse(
                _custom_cylinder(dia=2, dz=2.2, x=-8.017, y=i * 5, z=-layout.inch / 2)
            )
        part.Placement = obj.Placement
        obj.DrillPart = part
...
class mirror_mount_k05s1:
    ...
    def execute(self, obj):
        mesh = _import_stl(
            "POLARIS-K05S1-Step.stl", (90, 0, -90), (-4.514, 0.254, -0.254)
        )
        mesh.Placement = obj.Mesh.Placement
        obj.Mesh = mesh
...
class splitter_mount_b05g:
    ...
    def execute(self, obj):
        mesh = _import_stl(
            "POLARIS-B05G-Step.stl", (90, -0, 90), (-17.54, -5.313, -19.26)
        )
        mesh.Placement = obj.Mesh.Placement
        obj.Mesh = mesh

        part = _custom_cylinder(
            dia=bolt_8_32["tap_dia"], dz=drill_depth, x=-5, y=0, z=-layout.inch / 2
        )
        for i in [-1, 1]:
            part = part.fuse(
                _custom_cylinder(dia=2, dz=2.2, x=-5, y=i * 5, z=-layout.inch / 2)
            )
        part.Placement = obj.Placement
        obj.DrillPart = part
...
class mirror_mount_c05g:
    ...
    def execute(self, obj):
        mesh = _import_stl(
            "POLARIS-C05G-Step.stl", (90, -0, 90), (-18.94, -4.246, -15.2)
        )
        mesh.Placement = obj.Mesh.Placement
        obj.Mesh = mesh

        part = _custom_cylinder(
            dia=bolt_8_32["tap_dia"], dz=drill_depth, x=-6.35, y=0, z=-layout.inch / 2
        )
        for i in [-1, 1]:
            part = part.fuse(
                _custom_cylinder(dia=2, dz=2.2, x=-6.35, y=i * 5, z=-layout.inch / 2)
            )
        part.Placement = obj.Placement
        obj.DrillPart = part


# this class is Zhenyu editing
class KMS_MH_12:
    """
    KMSS mirror mount
...
class EOM_:
    ...
    def __init__(
        ...
        obj,
        drill=True,
        diffraction_angle=degrees(0.026),
        forward_direction=1,
        ...
        obj.addProperty("App::PropertyBool", "Drill").Drill = drill
        obj.addProperty(
            "App::PropertyAngle", "DiffractionAngle"
        ).DiffractionAngle = diffraction_angle
        obj.addProperty(
            "App::PropertyInteger", "ForwardDirection"
        ).ForwardDirection = forward_direction
        obj.addProperty(
            "App::PropertyInteger", "BackwardDirection"
        ).BackwardDirection = backward_direction
...
class grid_optics:
    ...
    def __init__(self, obj):
        ...
        for i in 1 + np.arange(Number_of_light_source):
            _add_linked_object(
                obj,
                "Laser_diode_LT230P-B_" + str(i),
                km05_50mm_laser_no_pad,
                pos_offset=(input_x, input_y + i * 1.5 * layout.inch, 0),
                rot_offset=(0, 0, layout.turn["left-up"]),
            )
            _add_linked_object(
                obj,
                "Laser_diode_LT230P-B_" + str(i),
                km05_50mm_laser_no_pad,
                pos_offset=(input_y + i * 1.5 * layout.inch, input_x, 0),
                rot_offset=(0, 0, layout.turn["left-up"]),
            )
            ...
            _add_linked_object(
                ...
                "mirror_" + str(i),
                circular_mirror,
                pos_offset=(base_dx - 5, input_y + i * 1.5 * layout.inch, 0),
                rot_offset=(
                    0,
                    0,
                    180,
                ),  # layout.turn['up-right'],
                mount_type=mirror_mount_k05s1,
            ...
            _add_linked_object(
                ...
                "mirror__" + str(i),
                circular_mirror,
                pos_offset=(input_y + i * 1.5 * layout.inch, base_dx - 5, 0),
                rot_offset=(0, 0, -90),  # layout.turn['up-right'],
                mount_type=mirror_mount_k05s1,
                mount_args=dict(thumbscrews=True),
            )
    ...
    def execute(self, obj):
        mesh = _import_stl(
            "grid_optics_fast_baseplate.stl", rotate=(0, 0, 0), translate=(0, 0, 0)
        )
...
class grid_beamsplitter_lying_down:
    ...
    def __init__(self, obj):
        ...
        for i in range(6):
            for j in range(6):
                _add_linked_object(
                    obj,
                    "cube_splitter" + str(i) + str(j),
                    cube_splitter,
                    pos_offset=(
                        (2 * i + 1.25) * layout.inch,
                        -18.0,
                        -(2 * j + 1.25) * layout.inch,
                    ),
                    rot_offset=(0, 0, 45),
                )
    ...
    def execute(self, obj):
        mesh = _import_stl(
            "grid_beamsplitter_lying_down_baseplate_mount.stl",
            rotate=(270, 0, 0),
            translate=(0, 0, 0),
        )
...
class grid_mirror_lying_down:
    ...
    def __init__(self, obj):
        ...
        for i in range(6):
            for j in range(6):
                ...
                _add_linked_object(
                    obj,
                    "mirror_mount" + str(i) + str(j),
                    mirror_mount_km05_lying_down,
                    pos_offset=(
                        (2 * i + 1.25) * layout.inch,
                        -18.0,
                        -(2 * j + 1.25) * layout.inch,
    ...
    def execute(self, obj):
        mesh = _import_stl(
            "grid_mirror_lying_down_baseplate.stl",
            rotate=(270, 0, 0),
            translate=(0, 0, 0),
        )
...
class grid_waveplate_lying_down:
    ...
    def execute(self, obj):
        mesh = _import_stl(
            "grid_waveplate_lying_down_baseplate.stl",
            rotate=(270, 0, 0),
            translate=(0, 0, 0),
        )
...
class prism_mount_km05pm:
    ...
    def __init__(self, obj, drill=True, thumbscrews=False, bolt_length=15, arm=True):
        ...
        if thumbscrews:
            _add_linked_object(
                ...
                "Upper Thumbscrew",
                thumbscrew_hkts_5_64,
                pos_offset=(-19.05, 6.985, 15.49),
            )
            _add_linked_object(
                obj,
                "Lower Thumbscrew",
                thumbscrew_hkts_5_64,
                pos_offset=(-19.05, -12.83, -4.318),
            )
    ...
    def execute(self, obj):
        ...
        if obj.Arm:
            mesh = _import_stl(
                "KM05PM-Step-No-Plate.stl", (90, -0, 90), (-6.425, -4.069, 6.086)
            )
        else:
            mesh = _import_stl(
                "KM05PM-Step-NoArm.stl", (90, 0, 90), (-3.25, -8.26, 10.4)
            )
        mesh.Placement = obj.Mesh.Placement
        obj.Mesh = mesh
        ...
        part = part.fuse(
            _custom_cylinder(
                ...
                dz=drill_depth,
                head_dia=bolt_8_32["head_dia"],
                head_dz=drill_depth - obj.BoltLength.Value,
                x=-15.8,
                y=-2.921,
                z=-9.144 - drill_depth,
                dir=(0, 0, 1),
...
class grating_mount_on_km05pm:
    ...
    def __init__(
        ...
        obj.addProperty(
            "App::PropertyAngle", "LittrowAngle"
        ).LittrowAngle = littrow_angle
    ...
    def execute(self, obj):
        ...
        temp = _custom_box(
            dx=ref_len * cos(beam_angle) + 6 + 3.2,
            dy=dy / sin(lit_angle) + 10,
            dz=dz,
            x=-cut_x,
            y=-(dx - cut_x) * cos(lit_angle),
            z=-6,
            dir=(-1, 1, 1),
        )
        temp.rotate(
        ...
        part = part.cut(
            _custom_box(
                dx=8, dy=16, dz=dz - 4, x=extra_x, y=dy + extra_y, z=-6, dir=(-1, -1, 1)
            )
        )
        part.translate(
            App.Vector(-extra_x, -12.7 / 2 * sin(lit_angle) - 6 * cos(lit_angle), 0)
        )
        part = part.fuse(part)
        part = part.cut(
            _custom_cylinder(
                dia=bolt_4_40["clear_dia"],
                dz=4,
                head_dia=bolt_4_40["head_dia"],
                head_dz=2,
                x=-3.175,
                y=8,
                z=-6,
...
class grating_mount_on_km05pm_no_arm:
    ...
    def __init__(
        ...
        _add_linked_object(
            ...
            "Mount MK05PM",
            prism_mount_km05pm,
            pos_offset=(-3.175, 8, 4.064 - INCH / 2),
            rot_offset=(0, 0, 180),
            arm=False,
            drill=drill,
            **mount_args,
...
class mount_tsd_405sluu:
    ...
    def execute(self, obj):
        ...
        for x, y in [
            ...
            part = part.fuse(
                _custom_cylinder(
                    dia=bolt_4_40["tap_dia"], dz=drill_depth, x=x, y=y, z=-62
                )
            )
        part.Placement = obj.Placement
        obj.DrillPart = part
...
class fiberport_mount_km05:
    ...
    def __init__(self, obj, drill=True, mount_args=dict()):
        ...
        _add_linked_object(
            obj, "Mount", mirror_mount_km05, pos_offset=(0, 0, 0), **mount_args
        )
        _add_linked_object(
            obj, "Fiber Adapter", fiber_adapter_sm05fca2, pos_offset=(1.524, 0, 0)
        )
        _add_linked_object(
            obj, "Lens Tube", lens_tube_sm05l05, pos_offset=(1.524 + 3.812, 0, 0)
        )
        _add_linked_object(
            obj, "Lens Adapter", lens_adapter_s05tm09, pos_offset=(1.524 + 5, 0, 0)
        )
        _add_linked_object(
            obj, "Lens", mounted_lens_c220tmda, pos_offset=(1.524 + 3.167 + 5, 0, 0)
        )
...
class fiberport_mount_ks1t:
    ...
    def __init__(self, obj, drill=True, mount_args=dict()):
        ...
        _add_linked_object(
            obj, "Mount", mirror_mount_ks1t, pos_offset=(0, 0, 0), **mount_args
        )
        _add_linked_object(
            obj, "Fiber Adapter", fiber_adapter_sm1fca2, pos_offset=(-3, 0, 0)
        )
        _add_linked_object(obj, "Lens Tube", lens_tube_sm1l05, pos_offset=(0, 0, 0))
        _add_linked_object(
            obj, "Lens Adapter", lens_adapter_s1tm09, pos_offset=(1.524 + 6, 0, 0)
        )
        _add_linked_object(
            obj, "Lens", mounted_lens_c220tmda, pos_offset=(1.524 + 2, 0, 0)
        )
...
class km05_50mm_laser:
    ...
    def __init__(
        ...
        tec_thickness=4,
        mount_args=dict(),
        upper_plate_args=dict(),
        lower_plate_args=dict(),
    ):
        ...
        obj.addProperty("App::PropertyBool", "Drill").Drill = drill
        obj.addProperty(
            "App::PropertyLength", "TecThickness"
        ).TecThickness = tec_thickness
        obj.ViewObject.ShapeColor = misc_color
        ...
        _add_linked_object(
            ...
            pos_offset=(
                dx - 4,
                0,
                -0.08 * INCH - tec_thickness - upper_plate.Thickness.Value,
            ),
            **lower_plate_args,
        )
...
class laser_cavity_mount:
    ...
    def __init__(
        ...
        mount = _add_linked_object(
            ...
            "Mount",
            fixed_mount_smr05,
            pos_offset=(dx + 5.334, 0, 16 - INCH / 2),
            drill=False,
            **mount_args,
        )
        grating = _add_linked_object(
            obj,
            "Grating",
            grating_mount_on_km05pm_no_arm,
            pos_offset=(dx + width * 3 / 4, 0, 16 - INCH / 2),
            drill=False,
            **grating_args,
        )
        upper_plate = _add_linked_object(
            obj,
            "Upper Plate",
            laser_cavity_mount_upper_plate,
            pos_offset=(
                dx - 8 + width / 2,
                0,
            ),
            drill_objs=[mount, grating],
            **upper_plate_args,
        )
        _add_linked_object(
            obj,
            "Lower Plate",
            laser_cavity_mount_lower_plate,
            pos_offset=(
                dx - 8 + width / 2,
                0,
                -tec_thickness - upper_plate.Thickness.Value,
            ),
            **lower_plate_args,
        )
...
class laser_cavity_mount_upper_plate:
    ...
    def __init__(
        self, obj, drill_objs, width=INCH, length=3 * INCH, thickness=0.25 * INCH
    ):
        obj.Proxy = self
        ViewProvider(obj.ViewObject)
        ...
        obj.addProperty(
            "App::PropertyLinkListHidden", "DrillObjects"
        ).DrillObjects = drill_objs
    ...
    def execute(self, obj):
        ...
        for x, y in [(1, 1), (1, -1), (-1, 1), (-1, -1)]:
            part = part.cut(
                _custom_cylinder(
                    dia=bolt_8_32["clear_dia"],
                    dz=obj.Thickness.Value,
                    x=(obj.Length.Value / 2 - 4) * x,
                    y=(obj.Width.Value / 2 - 4) * y,
                    z=-INCH / 2,
                    dir=(0, 0, -1),
                )
            )
        ...
        part = part.fuse(
            _custom_box(
                ...
                dy=5,
                dz=INCH / 2,
                x=part.BoundBox.XMin,
                y=(part.BoundBox.YMax + part.BoundBox.YMin) / 2,
                z=0,
                dir=(-1, 0, -1),
            )
        )
...
class km05_tec_upper_plate:
    ...
    def __init__(self, obj, drill_obj, width=INCH, thickness=0.25 * INCH):
        ...
        obj.addProperty("App::PropertyLength", "Thickness").Thickness = thickness
        obj.addProperty(
            "App::PropertyLinkHidden", "DrillObject"
        ).DrillObject = drill_obj
...
class mirror_mount_mk05:
    ...
    def execute(self, obj):
        mesh = _import_stl(
            "MK05-Step.stl",
            (90, -0, -90),
            (-22.91 - obj.ChildObjects[0].Thickness.Value, 26, -5.629),
        )
        mesh.Placement = obj.Mesh.Placement
        obj.Mesh = mesh

        part = _custom_cylinder(
            dia=bolt_4_40["tap_dia"],
            dz=drill_depth,
            head_dia=bolt_4_40["head_dia"],
            head_dz=drill_depth - 10,
            x=-5.562,
            y=0,
            z=-10.2 - drill_depth,
            dir=(0, 0, 1),
    ...
    def execute(self, obj):
        ...
        part = part.cut(
            _custom_box(
                dx=4,
                dy=15,
                dz=-layout.inch / 2 - obj.Mesh.BoundBox.ZMin,
                x=part.BoundBox.XMin,
                y=part.BoundBox.YMax,
        ...
        part = part.fuse(
            _custom_cylinder(
                ...
                dz=drill_depth,
                head_dia=bolt_4_40["head_dia"],
                head_dz=drill_depth - 5,
                x=-7.675,
                y=7.699,
                z=4.493 - 10.2 - drill_depth,
                dir=(0, 0, 1),
            )
        )
        part.Placement = obj.Placement
...
class grating_mount_on_mk05pm:
    ...
    def __init__(
        ...
        _add_linked_object(
            ...
            "Mirror",
            square_mirror,
            pos_offset=(self.dx, -12, 2),
            rot_offset=(0, 0, -obj.LittrowAngle.Value + 180),
            **mirror_args,
        )
    ...
    def execute(self, obj):
        ...
        part = _custom_box(
            dx=25 + self.dx, dy=35, dz=4, x=-3.048, y=17.91, z=0, dir=(1, -1, 1)
        )

        part = part.cut(
            _custom_box(dx=6, dy=8.1, dz=4, x=-3.048, y=17.91, z=0, dir=(1, -1, 1))
        )
        part = part.cut(
            _custom_cylinder(
                dia=bolt_4_40["clear_dia"], dz=4, x=0, y=0, z=0, dir=(0, 0, 1)
            )
        )
        part = part.cut(
            _custom_cylinder(
                dia=bolt_4_40["clear_dia"], dz=4, x=13.34, y=15.62, z=0, dir=(0, 0, 1)
            )
        )
        part.translate(App.Vector(-12, -4, -4))

        temp = _custom_box(dx=4, dy=12, dz=12, x=-6, y=0, z=0, dir=(-1, 0, 1))
        temp.rotate(App.Vector(0, 0, 0), App.Vector(0, 0, 1), -obj.LittrowAngle.Value)
        part = part.fuse(temp)
        temp = _custom_box(
            dx=4, dy=12, dz=12, x=self.dx + 3.2, y=-12, z=0, dir=(1, 0, 1)
        )
        temp.rotate(
            App.Vector(self.dx, -12, 0), App.Vector(0, 0, 1), -obj.LittrowAngle.Value
        )
        part = part.fuse(temp)
...
class lens_holder_l05g:
    ...
    def execute(self, obj):
        mesh = _import_stl(
            "POLARIS-L05G-Step.stl", (90, -0, 90), (-26.57, -13.29, -18.44)
        )
        mesh.Placement = obj.Mesh.Placement
        obj.Mesh = mesh

        part = _custom_cylinder(
            dia=bolt_8_32["tap_dia"], dz=drill_depth, x=-8, y=0, z=-layout.inch / 2
        )
        for i in [-1, 1]:
            part = part.fuse(
                _custom_box(
...
class pinhole_ida12:
    ...
    def execute(self, obj):
        ...
        part = _custom_box(
            dx=6.5,
            dy=15 + obj.ChildObjects[0].SlotLength.Value,
            dz=1,
            x=1.956,
            y=0,
            z=-layout.inch / 2,
...
class prism_mount_km100pm:
    ...
    def execute(self, obj):
        ...
        part = part.fuse(
            _custom_cylinder(
                dia=bolt_8_32["tap_dia"], dz=drill_depth, x=-14.02, y=12.63, z=17.5
            )
        )
        part.Placement = obj.Placement
        obj.DrillPart = part
...
class laser_box:
    ...
    def __init__(
        ...
        obj.addProperty(
            "App::PropertyLength", "MatThickness"
        ).MatThickness = mat_thickness
    ...
    def execute(self, obj):
        ...
        part = part.cut(
            _custom_box(
                dx=obj.Thickness.Value - INCH / 4 * 2,
                dy=obj.Width.Value - INCH / 4 * 2,
                dz=obj.Height.Value - INCH / 4,
                x=x_off,
                y=y_off,
                z=-3 / 2 * INCH - 3.95,
                dir=(0, 0, 1),
        ...
        part = part.cut(
            _custom_box(
                dx=obj.Thickness.Value / 2,
                dy=20,
                dz=70,
                x=x_off - obj.Thickness.Value / 2,
                y=y_off + 0,
                z=-2 * INCH - 3.95 - INCH,
                dir=(0, 0, 1),
            )
...
class laser_base:
    ...
    def execute(self, obj):
        ...
        part = _custom_box(
            ...
            x=x_off,
            y=y_off,
            z=-3 / 2 * INCH - 3.95 - obj.MatThickness.Value,
            dir=(0, 0, -1),
        )
        ...
        for x, y in [(-1, -1), (-1, 1), (1, -1), (1, 1)]:
            part = part.cut(
                _custom_cylinder(
                    ...
                    head_dz=8,
                    x=1.5 * INCH * x + x_off,
                    y=INCH * y + y_off - bolt_off,
                    z=-3 / 2 * INCH - 3.95 - INCH / 2,
                )
            )
...
class laser_mount_km100pm:
    ...
    def __init__(
        ...
        obj.addProperty(
            "App::PropertyLength", "CounterDepth"
        ).CounterDepth = counter_depth
        obj.addProperty(
            "App::PropertyLength", "ArmThickness"
        ).ArmThickness = arm_thickness
        obj.addProperty(
            "App::PropertyLength", "ArmClearance"
        ).ArmClearance = arm_clearance
        obj.addProperty(
            "App::PropertyLength", "StageThickness"
        ).StageThickness = stage_thickness
        ...
        _add_linked_object(
            ...
            "Grating",
            square_grating,
            pos_offset=(grating_dx + 35, -1, -2.7),
            rot_offset=(0, 0, 180 - obj.LittrowAngle.Value),
        )
        _add_linked_object(
            obj,
            "PZT",
            box,
            pos_offset=(grating_dx + 34.6 + 4.2, -4.2, -2.7),
            rot_offset=(0, 0, 180 - obj.LittrowAngle.Value),
        )
        _add_linked_object(
            obj,
            "Mirror",
            square_mirror,
            pos_offset=(mirror_dx + 37.5, gap + 4.5, -2.7),
            rot_offset=(0, 0, -obj.LittrowAngle.Value),
        )
        ...
        _add_linked_object(
            ...
            "TEC",
            TEC,
            pos_offset=(grating_dx + 11.1, 0, -33.7),
            rot_offset=(90, 90, 90),
        )
        _add_linked_object(
            obj,
            ...
            km05_tec_lower_plate,
            pos_offset=(
                2.032 + 13.96 - 3.8 - 13.96,
                0,
                -INCH / 4 - 6.3 - 4 - upper_plate.Thickness.Value,
            ),
            width=2.5 * INCH,
        )
    ...
    def execute(self, obj):
        ...
        temp = _custom_box(
            dx=ref_len * cos(beam_angle) + 6 + 3.2 + 3,
        ...
        temp.translate(
            App.Vector(
                -extra_x + 36, -17.7 / 2 * sin(lit_angle) - 6 * cos(lit_angle), 0
            )
        )
...
class mount_for_km100pm:
    ...
    def execute(self, obj):
        ...
        part = part.fuse(
            _custom_box(dx=stage_dx, dy=dy, dz=stage_dz, x=0, y=0, z=dz, dir=(1, 0, -1))
        )
        for ddy in [15.2, 38.1]:
            part = part.cut(
                _custom_box(
        ...
        for ddy in [0, -11.42, -26.65, -38.07]:
            part = part.cut(
                _custom_cylinder(
                    ...
                    dz=stage_dz,
                    head_dia=bolt_4_40["head_dia"],
                    head_dz=obj.CounterDepth.Value,
                    countersink=obj.Countersink,
                    x=11.25,
                    y=18.9 + ddy,
                    z=dz - 4,
                    dir=(0, 0, 1),
...
class isolator_670:
    ...
    def execute(self, obj):
        ...
        part = _custom_box(
            dx=80, dy=25, dz=5, x=0, y=0, z=-layout.inch / 2, fillet=5, dir=(0, 0, -1)
        )
        part.Placement = obj.Placement
        obj.DrillPart = part
...
class rb_cell_new:
    ...
    def execute(self, obj):
        ...
        base = _custom_box(dx=dx, dy=dy, dz=dz / 2, x=0, y=0, z=dz / 2, dir=(0, 0, -1))
        base = base.fuse(
            _custom_box(
                dx=dx,
                dy=base_dy,
        ...
        cover = _custom_box(dx=dx, dy=dy, dz=dz / 2, x=0, y=0, z=dz / 2, dir=(0, 0, 1))
        cover = cover.cut(
            _custom_box(
                dx=20, dy=dy / 2, dz=5, x=0, y=0, z=dz / 2 + 2.5, dir=(0, -1, 0)
            )
        )
        cell = _custom_cylinder(
            dia=cell_dia, dz=cell_dx, x=-cell_dx / 2, y=0, z=dz / 2, dir=(1, 0, 0)
        )
        cell = cell.fuse(
            _custom_cylinder(
                dia=end_dia, dz=10, x=-cell_dx / 2, y=0, z=dz / 2, dir=(1, 0, 0)
            )
        )
        cell = cell.fuse(
            _custom_cylinder(
        ...
        cell = cell.fuse(
            _custom_cylinder(dia=5, dz=dx, x=-dx / 2, y=0, z=dz / 2, dir=(1, 0, 0))
        )
        cell = cell.fuse(
            _custom_cylinder(
                dia=15, dz=cell_dia / 2 + 10, x=0, y=0, z=dz / 2, dir=(0, 1, 0)
            )
        )
        ...
        for x, y in [(-1, -1), (-1, 1), (1, -1), (1, 1)]:
            hole = _custom_cylinder(
                dia=bolt_8_32["clear_dia"],
                dz=dz / 2,
                x=x * (dx / 2 - wall_thickness / 2),
                y=y * (dy / 2 - wall_thickness / 2),
                z=dz,
                head_dia=bolt_8_32["head_dia"],
                head_dz=dz / 4,
            )
...
class photodetector_pda10a2:
    ...
    def __init__(self, obj, drill=True, adapter_args=dict()):
        ...
        _add_linked_object(
            ...
            "Surface Adapter",
            surface_adapter,
            pos_offset=(-10.54, 0, -25),
            **adapter_args,
        )
        _add_linked_object(
            obj, "Lens Tube", lens_tube_SM1L03, pos_offset=(-0.124, 0, -0)
        )
...
class lens_tube_SM1L03:
    ...
    def execute(self, obj):
        ...
        part = _bounding_box(
            obj, 2, 3, z_tol=True, min_offset=(0, 4, 0), max_offset=(0, -4, 0)
        )
        part.Placement = obj.Placement
        obj.DrillPart = part
...
class periscope:
    ...
    def __init__(
        ...
        _add_linked_object(
            ...
            "Lower Mirror",
            circular_mirror,
            rot_offset=((-1) ** invert * 90, -45, 0),
            pos_offset=(0, 0, obj.LowerHeight.Value + self.z_off),
            **mirror_args,
        )
        _add_linked_object(
            obj,
            "Upper Mirror",
            circular_mirror,
            rot_offset=((-1) ** invert * 90, 135, 0),
            pos_offset=(0, 0, obj.UpperHeight.Value + self.z_off),
            **mirror_args,
        )
    ...
    def execute(self, obj):
        width = (
            2 * INCH
        )  # Must be inch wide to keep periscope mirrors 1 inch from mount holes.
        fillet = 15
        part = _custom_box(
            dx=70, dy=width, dz=obj.UpperHeight.Value + 20, x=0, y=0, z=0
        )
        for i in [-1, 1]:
            part = part.cut(
                _custom_box(
            ...
            for y in [-INCH / 2, INCH / 2]:
                part = part.cut(
                    _custom_cylinder(
                        dia=bolt_14_20["clear_dia"] + 0.5,
                        dz=INCH + 5,
                        head_dia=bolt_14_20["head_dia"] + 0.5,
                        head_dz=10,
                        x=i * INCH,
                        y=y,
                        z=25,
        ...
        part.translate(
            App.Vector(0, (-1) ** obj.Invert * (width / 2 + INCH / 2), self.z_off)
        )
        part = part.fuse(part)
...
class periscope_for_redstone:
    ...
    def execute(self, obj):
        ...
        # obj.Mesh = mesh
        part = _custom_box(
            dx=210, dy=1.1 * width, dz=obj.UpperHeight.Value + 78, x=-20, y=-20, z=0
        )
        ...
        for i in range(6):  # lower
            part = part.fuse(
                _custom_box(
                    dx=1.7 * width,
                    dy=150 + 18 - (i + 1) * 25,
                    dz=obj.UpperHeight.Value + 95,
                    x=100 - (i + 1) * 35,
                    y=200 + width + (i + 1) * 12.5,
                    z=0,
                )
            )
        ...
        for i in range(6):  # upper
            part = part.fuse(
                _custom_box(
                    dx=210,
                    dy=1.2 * width,
                    dz=obj.UpperHeight.Value + 78 - (i + 1) * 23,
                    x=-20,
                    y=-20 + 1.1 * width * (i + 1),
                    z=-3,
                )
...
class thumbscrew_hkts_5_64:
    ...
    def execute(self, obj):
        ...
        part = _bounding_box(
            obj, 2, 3, z_tol=True, min_offset=(-6, 0, 0), max_offset=(-6, 0, 0)
        )
        part.Placement = obj.Placement
        obj.DrillPart = part
...
class Room_temp_chamber:
    ...
    def execute(self, obj):
        mesh = _import_stl(
            "Room_temp_chamber_step.stl", (0, 0, 0), (-48.89, 1.266, 0.813)
        )
        mesh.Placement = obj.Mesh.Placement
        obj.Mesh = mesh
...
class Room_temp_chamber_Mechanical:
    ...
    def execute(self, obj):
        mesh = _import_stl(
            "Room Temp Chamber Mechanical.stl", (0, 0, 0), (-33.46, -10.12, -59.69)
        )
        mesh.Placement = obj.Mesh.Placement
        obj.Mesh = mesh
...
class box:
    ...
    def __init__(
        self, obj, drill=True, thickness=3, width=10, height=10, part_number=""
    ):
        obj.Proxy = self
        ViewProvider(obj.ViewObject)
...
class square_grating:
    ...
    def __init__(
        self, obj, drill=True, thickness=6, width=12.7, height=12.7, part_number=""
    ):
        obj.Proxy = self
        ViewProvider(obj.ViewObject)
...
class circular_splitter:
    ...
    def __init__(
        ...
        if mount_type != None:
            _add_linked_object(
                obj, "Mount", mount_type, pos_offset=(-thickness, 0, 0), **mount_args
            )
    ...
    def __init__(
        ...
        if mount_type != None:
            _add_linked_object(
                ...
                "Mount",
                mount_type,
                pos_offset=(0, 0, -cube_size / 2),
                **mount_args,
            )

    def execute(self, obj):
        part = _custom_box(
            dx=obj.CubeSize.Value,
            dy=obj.CubeSize.Value,
            dz=obj.CubeSize.Value,
            x=0,
            y=0,
            z=0,
            dir=(0, 0, 0),
    ...
    def __init__(
        ...
        if mount_type != None:
            _add_linked_object(
                ...
                "Mount",
                mount_type,
                pos_offset=(thickness / 2, 0, -height / 2),
                mount_hole_dy=width + 10,
                cube_dy=width,
                cube_dz=height,
                cube_dx=thickness,
                slots=slots,
...
class square_mirror:
    ...
    def __init__(
        self, obj, drill=True, thickness=3.2, width=12.7, height=12.7, part_number=""
    ):
        obj.Proxy = self
        ViewProvider(obj.ViewObject)
        ...
        if str(prop) == "BasePlacement":
            ...
            if hasattr(obj, "ChildObjects"):
                for child in obj.ChildObjects:
                    child.BasePlacement.Base = (
                        obj.BasePlacement.Base + child.RelativePlacement.Base
                    )
                    if hasattr(child, "Angle"):
                        obj.BasePlacement.Rotation = App.Rotation(
                            App.Vector(0, 0, 1), obj.Angle
                        )
                    else:
                        child.BasePlacement = App.Placement(
                            child.BasePlacement.Base,
                            obj.BasePlacement.Rotation,
                            -child.RelativePlacement.Base,
                        )
                        child.BasePlacement.Rotation = (
                            child.BasePlacement.Rotation.multiply(
                                child.RelativePlacement.Rotation
                            )
                        )
