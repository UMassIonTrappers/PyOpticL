...
class baseplate:
    ...
    def __init__(
        ...
        obj.addProperty("App::PropertyLength", "dx").dx = (
            dx  # define and set baseplate dimentions
        )
    ...
    def place_element(self, name, obj_class, x, y, angle, optional=False, **args):
        ...
        obj.addProperty("App::PropertyLinkHidden", "Baseplate").Baseplate = getattr(
            App.ActiveDocument, self.active_baseplate
        )
        obj.Label = name
        obj_class(obj, **args)
        ...
        obj.BasePlacement = App.Placement(
            App.Vector(x, y, 0), App.Rotation(angle, 0, 0), App.Vector(0, 0, 0)
        )
    ...
    def place_element_general(
        ...
        **args
        ...
        obj.BasePlacement = App.Placement(
            App.Vector(x, y, z),
            App.Rotation(angle_x, angle_y, angle_z),
            App.Vector(0, 0, 0),
        )
    ...
    def place_element_relative(
        ...
        **args
        ...
        obj.addProperty("App::PropertyLinkHidden", "RelativeParent").RelativeParent = (
            rel_obj
        )
    ...
    def execute(self, obj):
            ...
            for i in App.ActiveDocument.Objects:
                if hasattr(i, "Baseplate") and i.Baseplate == obj:
                    ...
                    if hasattr(obj_body, "BoundBox") and hasattr(i, "BasePlacement"):
                        ...
                        obj.xOffset = min(
                            obj.xOffset.Value, bound.XMin - obj.AutosizeTol.Value
                        )
                        obj.yOffset = min(
                            obj.yOffset.Value, bound.YMin - obj.AutosizeTol.Value
                        )
                        obj.dx = max(
                            obj.dx.Value,
                            bound.XMax + obj.AutosizeTol.Value - obj.xOffset.Value,
                        )
                        obj.dy = max(
                            obj.dy.Value,
                            bound.YMax + obj.AutosizeTol.Value - obj.yOffset.Value,
                        )
                        print(i.Name, obj.dy)
        ...
        part = Part.makeBox(
            obj.dx.Value - 2 * obj.Gap.Value,
            obj.dy.Value - 2 * obj.Gap.Value,
            obj.dz.Value,
            App.Vector(
                obj.Gap.Value + obj.xOffset.Value,
                obj.Gap.Value + obj.yOffset.Value,
                -obj.dz.Value - obj.OpticsDz.Value,
            ),
        )
            ...
            for i in obj.xSplits:
                part = part.cut(
                    Part.makeBox(
                        ...
                        obj.dz.Value,
                        App.Vector(
                            i - obj.Gap.Value + obj.xOffset.Value,
                            obj.Gap.Value + obj.yOffset.Value,
                            -obj.dz.Value - obj.OpticsDz.Value,
                        ),
                    )
            ...
            for i in obj.ySplits:
                part = part.cut(
                    Part.makeBox(
                        ...
                        App.Vector(
                            obj.Gap.Value + obj.xOffset.Value,
                            i - obj.Gap.Value + obj.yOffset.Value,
                            -obj.dz.Value - obj.OpticsDz.Value,
                        ),
                    )
                )
            ...
            for i in App.ActiveDocument.Objects:
                if hasattr(i, "DrillPart"):
                    if i.Drill and i.Baseplate == obj:
                        ...
                        part = part.cut(drill)
        if obj.CutLabel != "":
            face = Draft.make_shapestring(
                obj.CutLabel,
                str(Path(__file__).parent.resolve()) + "/font/OpenSans-Regular.ttf",
                5,
            )
            if obj.InvertLabel:
                face.Placement.Base = App.Vector(
                    obj.Gap.Value + obj.xOffset.Value,
                    obj.dy.Value + obj.yOffset.Value - obj.Gap.Value - 2,
                    -obj.OpticsDz.Value - 6,
                )
                face.Placement.Rotation = App.Rotation(
                    App.Vector(0, 0, 1), -90
                ) * App.Rotation(App.Vector(1, 0, 0), 90)
                text = face.Shape.extrude(App.Vector(0.5, 0, 0))
            else:
                face.Placement.Base = App.Vector(
                    obj.Gap.Value + obj.xOffset.Value + 2,
                    obj.Gap.Value + obj.yOffset.Value,
                    -obj.OpticsDz.Value - 6,
                )
...
def place_element_on_table(name, obj_class, x, y, angle, z=0, **args):
    ...
    obj.addProperty("App::PropertyPlacement", "BasePlacement")
    obj.BasePlacement = App.Placement(
        App.Vector(x * inch, y * inch, z * inch),
        App.Rotation(angle, 0, 0),
        App.Vector(0, 0, 0),
    )
    return obj
...
class baseplate_cover:
    ...
    def __init__(self, obj, baseplate, dz, wall_thickness=10, beam_tol=5, drill=True):
        ...
        obj.addProperty("App::PropertyLength", "dz").dz = dz
        obj.addProperty("App::PropertyLength", "WallThickness").WallThickness = (
            wall_thickness
        )
    ...
    def execute(self, obj):
        ...
        part = Part.makeBox(
            baseplate.dx.Value - 2 * baseplate.Gap.Value,
            baseplate.dy.Value - 2 * baseplate.Gap.Value,
            obj.dz.Value,
            App.Vector(
                baseplate.Gap.Value + baseplate.xOffset.Value,
                baseplate.Gap.Value + baseplate.yOffset.Value,
                -baseplate.OpticsDz.Value,
            ),
        )
        part = part.fuse(
            Part.makeBox(
                baseplate.dx.Value
                - 2 * baseplate.Gap.Value
                ...
                App.Vector(
                    baseplate.Gap.Value
                    + baseplate.xOffset.Value
                    + obj.WallThickness.Value / 2
                    + 0.5,
                    baseplate.Gap.Value
                    + baseplate.yOffset.Value
                    + obj.WallThickness.Value / 2
                    + 0.5,
                    -baseplate.OpticsDz.Value - 1,
                ),
            )
        )
        part = part.cut(
            Part.makeBox(
                baseplate.dx.Value
                - 2 * baseplate.Gap.Value
                - 2 * obj.WallThickness.Value
                + 1,
                baseplate.dy.Value
                - 2 * baseplate.Gap.Value
                - 2 * obj.WallThickness.Value
                + 1,
                obj.dz.Value - obj.WallThickness.Value + 1,
                App.Vector(
                    baseplate.Gap.Value
                    + baseplate.xOffset.Value
                    + obj.WallThickness.Value
        ...
        temp = Part.makeBox(
            baseplate.dx.Value - 2 * baseplate.Gap.Value - obj.WallThickness.Value,
            baseplate.dy.Value - 2 * baseplate.Gap.Value - obj.WallThickness.Value,
            1.5,
            App.Vector(
                baseplate.Gap.Value
                + baseplate.xOffset.Value
            ...
            for i in App.ActiveDocument.Objects:
                if isinstance(i.Proxy, laser.beam_path) and i.Baseplate == baseplate:
                    ...
                    for shape in exploded:
                        drill = optomech._bounding_box(
                            shape, obj.BeamTol.Value, 2, z_tol=True, plate_off=-1
                        )
        ...
        if baseplate.CutLabel != "":
            ...
            if baseplate.InvertLabel:
                face.Placement.Base = App.Vector(
                    baseplate.Gap.Value,
                    baseplate.dy.Value - baseplate.Gap.Value - 2,
                    -baseplate.OpticsDz.Value - 6,
                )
                face.Placement.Rotation = App.Rotation(
                    App.Vector(0, 0, 1), -90
                ) * App.Rotation(App.Vector(1, 0, 0), 90)
...
class table_grid:
    ...
    def __init__(self, dx, dy, z_off=-3 / 2 * inch):
        ...
        obj.addProperty("App::PropertyLinkListChild", "ChildObjects").ChildObjects += [
            self.holes
        ]
        ViewProvider(obj.ViewObject)
        obj.Proxy = self
    ...
    def execute(self, obj):
        part = Part.makeBox(
            ...
            self.dy * inch,
            inch / 4,
            App.Vector(0, 0, self.z_off - inch / 4),
        )
        holes = []
        for x in range(self.dx):
            for y in range(self.dy):
                for z in [self.z_off + 1e-2, self.z_off - inch / 4 - 1e-2]:
                    holes.append(
                        Part.Circle(
                            App.Vector((x + 0.5) * inch, (y + 0.5) * inch, z),
                            App.Vector(0, 0, 1),
                            inch / 10,
                        )
                    )
