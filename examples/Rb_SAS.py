...
mount_holes = [(0,0), (6, 0), (1, 3), (7, 1), (13, 0), (13, 3)]

def Rb_SAS(x=0, y=0, angle=0, mirror=optomech.mirror_mount_km05, x_split=0, thumbscrews=False):

    baseplate = layout.baseplate(base_dx, base_dy, base_dz, x=x, y=y, angle=angle,
                                 gap=gap, name=name, label=label, mount_holes=mount_holes, x_split=x_split)
    ...
    baseplate.place_element_along_beam("probe_mirror_3", mirror, beam,
    ...
    baseplate.place_element_along_beam("Rb_gas_cell", optomech.rb_cell, beam,
                                       beam_index=0b111, distance=2.75*layout.inch, angle=layout.cardinal['right'])

    baseplate.place_element_along_beam("Beam_Splitter_2", optomech.pbs_on_skate_mount, beam,
                                       beam_index=0b111, distance=2.75*layout.inch, angle=layout.cardinal['left'],
                                       invert=False)

    baseplate.place_element_along_beam("Photodetector", optomech.photodetector_pda10a2, beam,
                                       beam_index=0b1110, distance=1*layout.inch, angle=layout.cardinal['right'])
    ...
    baseplate.place_element_along_beam("pump_mirror_1", mirror, beam,
                                       beam_index=0b110, distance=2.75*layout.inch, angle=layout.turn['left-up'],
                                       thumbscrews=thumbscrews) 
