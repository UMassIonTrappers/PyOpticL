...
from scalable_rb_sas.parameters import get_scale_parameters
...
def rb_sas():
    baseplate = Component(
        ...
        definition=Baseplate(
            ...
            mount_holes=scale_params["mount_holes"],
        ),
    )

    beam = baseplate.add(
        BeamPath(label="Beam", wavelength=780, waist=scale_params["beam_waist"]),
        position=(dim(16, "grid") * scale_params["overall_scale"], dim(0.5, "in"), 0),
    ...
    beam.add(
        Component(
            label="Pump/Probe Splitter", definition=scale_params["circular_sampler"]
        ),
        beam_index=0b11,
