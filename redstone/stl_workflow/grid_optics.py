...
def grid_mirror_light(Number_of_light_source = 13, x=0, y=0, angle=0, mirror=optomech.mirror_mount_k05s1, x_split=False, thumbscrews=True):
    if Number_of_light_source > 8:
        base_dx = Number_of_light_source* 2.25 * layout.inch + 2
        base_dy = Number_of_light_source* 2.25 * layout.inch + 2
    else:
        base_dx = Number_of_light_source* 2.3 * layout.inch + 3
        base_dy = Number_of_light_source* 2.3 * layout.inch + 3
    ...
    for i in 1 + np.arange(Number_of_light_source):
        ...
        baseplate.place_element('Laser_diode_LT230P-B_' + str(i), optomech.km05_50mm_laser,
                                x=input_x, y=input_y + i * 2.1 * layout.inch, angle=layout.turn['left-up'])
        
        baseplate.place_element_along_beam('mirror_' + str(i), optomech.circular_mirror, beam,
                                    beam_index=0b1, distance=1.41421356 * (2.16 * Number_of_light_source-2.1 * i) * layout.inch , angle=layout.cardinal['down'],  #layout.turn['up-right'],
        ...
        baseplate.place_element_along_beam('mirror__' + str(i), optomech.circular_mirror, beam,
                                    beam_index=0b1, distance=1.41421356 * (2.1 * i-1.0) * layout.inch , angle=layout.cardinal['left'],  #layout.turn['up-right'],
                                    mount_type=mirror, mount_args=dict(thumbscrews=thumbscrews))
        
        baseplate.place_element_along_beam('Laser_diode_LT230P-B__' + str(i), optomech.km05_50mm_laser, beam,
                                beam_index=0b1, distance=1.41421356 * (2.16 * Number_of_light_source-2.1 * i) * layout.inch ,angle=layout.turn['left-up'])
