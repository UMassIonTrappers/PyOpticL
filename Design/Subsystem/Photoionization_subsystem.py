...
def PI_subsystem_commercial(x=0, y=0, angle=0, thumbscrews=True):
    ...
    sourcebox(x=-1 + x, y=-6 + y, angle = 0 + angle) # modeling of a commercial laser
    layout.place_element_on_table("Periscope", optomech.periscope, x=1.5 + x, y=6 + y,z=0, angle=layout.cardinal['up'], mirror_args=dict(mount_type=optomech.mirror_mount_k05s1))
