...
def laser_cooling_subsystem(
    ...
    **args_for_doublepass
    ...
    ECDL(**p(21.3, 4), angle=90 + angle, littrow_angle=littrow_angle)
    ECDL_isolator_baseplate(**p(24, 9), angle=layout.cardinal["up"] + angle)
    telescope(**p(24, 16), angle=90 + angle)
    Rb_SAS(**p(10, 20), angle=0 + angle, thumbscrews=thumbscrews)
    doublepass_f50(
        **p(17, 26), angle=0 + angle, thumbscrews=thumbscrews, **args_for_doublepass
    )
