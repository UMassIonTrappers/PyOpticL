...
def singlepass_mirrored(x=0, y=0, angle=270, mirror=optomech.mirror_mount_km05, x_split=False, thumbscrews=True, add_box = False):
    D1 = 1 * layout.inch
    D2 = 0
        # Dimension of the baseplate
    base_dx = 8.25*layout.inch  # 8.25
    ...
    if add_box:
        ...
        D1 = 1.85 * layout.inch
        D2 = 0.7 * layout.inch
        mh = [(3,4.5),(8,3.5),(3,3.5),(3,1.5),(2,.5),(8,2.5)]
    # Difining the baseplate
    baseplate = layout.baseplate(base_dx, base_dy, base_dz, x=x, y=y, angle=angle,
                                gap=gap, mount_holes=mh,
                                name=name, label=label, x_splits=[0*layout.inch], y_offset =0)
    # Adding the beam to the baseplate
    beam = baseplate.add_beam_path(base_dx-input_x, base_dy - gap - 5 , layout.cardinal['down'])
    ...
    # but it is usually quite small
    if add_box:
        baseplate.add_beam_path(136, 0, layout.cardinal['up'], color = (0.0, 0.0, 0.0)) # used for a open window for RF port
    # Lens 2
    lens = baseplate.place_element_along_beam("Lens f100mm AB coat", optomech.circular_lens, beam,
...
if __name__ == "__main__":
    ...
    singlepass_mirrored(x = 5, add_box=False)
    layout.redraw()